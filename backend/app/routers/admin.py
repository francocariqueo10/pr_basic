...
def get_team_pool(db: Session = Depends(get_db)):
    """Return assigned and available official teams."""
...
def draw_team(team_id: int, db: Session = Depends(get_db)):
    """Randomly assign an available official team to the given player."""
...
def clear_team_draw(team_id: int, db: Session = Depends(get_db)):
    """Release the official team assignment for the given player."""
    team = db.query(Team).filter(Team.id == team_id).first()
    if not team:
        raise HTTPException(status_code=404, detail="Jugador no encontrado")
...
def reset_team_pool(db: Session = Depends(get_db)):
    """Clear all official team assignments."""
